...
(function($){
	$.fn.crossword = function(entryData) {
			...
			*/
			...
			this.after('<div id="puzzle-clues"><h1>Petunjuk</h1><h2>Across</h2><ol id="across"></ol><h2>Down</h2><ol id="down"></ol></div>');
			...
			var puzInit = {
				...
				*/
				buildEntries: function() {
					...
					for (var i=1, p = entryCount+1; i < p; i++) {
						console.log('+++++++++++++++++++++++++++++++++');
						
						$groupedLights = $('.entry-' + i);

						console.log("Iteration: " + i);
						// console.log(puzz.data[i].answer);
						// console.log($('.entry-' + i +':eq(0) span').length);
						// console.log("Position: " + puzz.data[i].position);
						
						if(!$('.entry-' + i +':eq(0) span').length){
							console.log("entry-"+$('.entry-' + i +':eq(0) span').length);
							console.log($groupedLights);
							$groupedLights.eq(0)
								.append('<span>' + puzz.data[i-1].position + '</span>');
							$groupedLights.eq(0).find("input").addClass("first-col");
							$groupedLights.eq(0).find("input").val(puzz.data[i-1].answer.substring(0, 1));
						}
